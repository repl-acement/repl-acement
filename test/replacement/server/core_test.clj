(ns replacement.server.core-test
	...
	(:import (clojure.lang DynamicClassLoader TaggedLiteral)))
	...
	(testing "Various comment styles"
		(let [prepl-opts (->prepl-client)
			...
			(let [input "(comment 42)"
				...
				(is (= :ret tag))
				(is (read-string form) (read-string input))
				(is (= {:column 1 :line 1} (meta (read-string form))))
				(is (nil? (read-string val))))
...
(deftest ^:graceful-fail-tests graceful-fail-tests
	(testing "Test graceful failures for syntax and spec errors"
		(let [prepl-opts (->prepl-client)
					{:keys [tag val exception]} (sync-results prepl-opts "(prn \"000")]
			(is (= :ret tag))
			(is (and (map? val)
							 (true? exception)))

			(let [{:keys [tag val exception]} (sync-results prepl-opts "(")]
				(is (= :ret tag))
				(is (and (map? val)
								 (true? exception))))
