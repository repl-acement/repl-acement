(ns nextjournal.clojure-mode.demo.ws
  (:require
    goog.date.Date
...
(defmethod -event-msg-handler :chsk/state
  ...
  (let [[_ new-state-map] (have vector? ?data)]
    ...
    (re-frame/dispatch [:nextjournal.clojure-mode.demo.events/client-uid (:uid new-state-map)])
    (re-frame/dispatch [:nextjournal.clojure-mode.demo.events/network-status (:open? new-state-map)])))
...
(defmethod -event-msg-handler :chsk/recv
  ...
  (let [push-event (first ?data)
    ...
    (cond
      ...
      (re-frame/dispatch [:nextjournal.clojure-mode.demo.events/users push-data])

      (= push-event :replacement/eval)
      (re-frame/dispatch [:nextjournal.clojure-mode.demo.events/eval-result push-data])

      (= push-event :replacement/analysis)
      (re-frame/dispatch [:nextjournal.clojure-mode.demo.events/analysis push-data])
...
(defmethod -event-msg-handler :chsk/handshake
  ...
  (re-frame/dispatch [:nextjournal.clojure-mode.demo.events/team-bootstrap]))
